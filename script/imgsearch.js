...
class ModalImagePreviewElement {
    ...
    #eventListenerClearSelected = () => {
        this.clearThisFromSelected.call(this);
    };
    ...
    set #selected(value) {
        ...
        if (value) {
            ...
            selectedImgModal.addCustomEventListener('clear', this.#eventListenerClearSelected);
        } else {
            selectedImgModal.delete(this.#imageElement);
            selectedImgModal.removeCustomEventListener('clear', this.#eventListenerClearSelected);
        }
    }
    ...
    constructor(imageElement = null) {
        ...
        this.divSelectionMark = domElementMixIn(document.createElement('div'))
            .setPropertiesValues(this._options.divSelectionMark.initial);
        this.divOpenLightbox = domElementMixIn(document.createElement('div'))
            .setPropertiesValues(this._options.divOpenLightbox.initial);
        this.divOpenLightboxIcon = domElementMixIn(document.createElement('div'))
            .setPropertiesValues(this._options.divOpenLightboxIcon.initial);
        this.divSelect = domElementMixIn(document.createElement('div'))
            .setPropertiesValues(this._options.divSelect.initial);
        this.divSelectIcon = domElementMixIn(document.createElement('div'))
...
class WebQueryProvider {
    ...
    async getPageResults({page = null, navigate = null,} = {}) {
        ...
        if (page != null) {
            if (navigate == 'next') {
                if ((this.#resultRequestImageExtendeds.length >= (page + 1) * 9) || this.#canSearchFurther) page++;
                else return null;
            }
        ...
        try {
            ...
            this.resultObj.imgExtResultArr = this.#resultRequestImageExtendeds.slice((page - 1) * 9, page * 9);
...
function nextSearch() {
    (async () => {
        ...
        queryToDo.getPageResults(queryToDo.resultObj)
            .then((result) => {
                fillImagesPreviewModal(result?.imgExtResultArr);
                if (result != null) mainDOMElements.modal.inputTextSearch.value = result.query;
            }).then(() => {
            mainDOMElements.loaderScreen.style.display = 'none';
            mainDOMElements.modal.inputTextSearch.value = queryToDo.resultObj.query;
        });
...
function fillImagesPreviewModal(resultsImgArr) {
    ...
    for (let imgExt of resultsImgArr) {
        mainDOMElements.modal.imgGridContainer.appendChild(new ModalImagePreviewElement(imgExt).div);
    }
}